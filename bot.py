...
# Apply nest_asyncio to avoid event loop conflicts in cloud environments
nest_asyncio.apply()

# Fetch and validate environment variables with fallback
...
if not BOT_TOKEN or API_ID == 0 or not API_HASH:
    print("ERROR: Telegram API_ID, API_HASH, or BOT_TOKEN is missing or invalid in environment variables!")

# Initialize Pyrogram client with in_memory=True to prevent session lock errors on cloud hosts
app = Client(
    "kb_movie_bot",
    api_id=API_ID,
    api_hash=API_HASH,
...
# Start command handler (case_sensitive=False handles /start, /Start, /START perfectly)
...
if __name__ == "__main__":
    print("🤖 Movie Bot is starting up with absolute error protection...")
    app.run()
